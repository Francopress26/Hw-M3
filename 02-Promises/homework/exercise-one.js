...
function problemC () {
  ...
  promisifiedReadFile('poem-one/stanza-02.txt')
    .then(stanza2=> {
      blue(stanza2);
      return promisifiedReadFile('poem-one/stanza-03.txt') // Si yo quiero que el proximo .then tome el valor de la lectura tengo q retornarla
    }).then(stanza3 => { // Stanza 3 vale la lectura del read uso llaves para retornar un obj o para hacer mas de 1 accion creo
      blue(stanza3);
      console.log("done")
...
function problemD () {
  ...
  // promisifiedReadFile('poem-one/wrong-file-name.txt')
    // .then(stanza4 => blue(stanza4), err => magenta(new Error(err))) // Una alternativa, teniendo en cuenta que las promesas pueden recibir 2 parametros
